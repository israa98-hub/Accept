...
def account():
    ...
    def newbord():
        ...
        def convertTbinary(board_name, name):
            global updated_name
            # a = open(name, 'rb')
            import shutil
            try:
                shutil.copy(name, 'Accept')
                tmp = name.split('/')
                updated_name = tmp[len(tmp) - 1]
        ...
        def insertimag(num):
            ...
            def saveimb():
                ...
                if num == 1:
                    cursor.execute("UPDATE 'boards' SET nameim1=?,sound1=?,image1=? WHERE name=? ",
                                   (nameim.get(), vvo, updated_name, txtbord.get()))
                    conn.commit()
                elif num == 2:
            ...
            Button(creatimframe, text="Add voice  ", bg="white", height="5", width="10", font="20",
            ...
            Button(creatimframe, text="Add pic ", bg="white", height="5", width="10", font="20",
                   command=lambda: convertTbinary(num,
                                                  filedialog.askopenfilename(filetypes=[("Image File", '.jpg')]))).grid(
                row=4, column=2, sticky=W, padx=90, pady=60)
            ok = tk.Button(creatimframe, text=" save ", bg="white", height="3", width="5", font="20",
        ...
        def creatbord():
            ...
            Button(isShow, text="Save", bg="white", height="3", width="10", font="100",
                   command=lambda: mainpage()).grid(row=3, column=4, sticky=W, padx=170, pady=20)
...
'''
    ...
    def mainpage():
        ...
        Button(mainframe, text="Create a new board!", bg="white", font=fnt, command=newbord).grid(row=4, column=1,
                                                                                                  padx=300, pady=10)
        Button(mainframe, text="An existing boards", bg="white", font=fnt, command=allboards).grid(row=2, column=1,
                                                                                                   padx=200, pady=10)
